...
| **Sidebar** | **200px** (expanded) / **72px** (collapsed) | Navigasi menu utama. Expanded: ikon + label teks. Collapsed: ikon saja. Toggle dengan tombol `‹/›`. |
...
1. Lebar: **200px** (expanded, default) / **72px** (collapsed). Transisi: `width 0.25s ease`.
2. Background: warna `#131A35` (navy gelap). Tidak boleh menggunakan warna lain.
3. **Expanded**: item navigasi menampilkan **ikon + label teks** secara horizontal. Layout `flex-direction: row`, `gap: 10px`, `padding: 10px 12px`.
4. **Collapsed**: item navigasi menampilkan **ikon saja**, label tersembunyi (`display: none`), item centered.
5. Item aktif: `background: rgba(37,99,235,0.25); border-radius: 10px`. Warna ikon: `#93C5FD`. Warna label: `#93C5FD; font-weight: 600`.
6. Item normal: ikon `rgba(255,255,255,0.4)`, label `rgba(255,255,255,0.5)`. Hover: ikon `rgba(255,255,255,0.7)`, label `rgba(255,255,255,0.9)`.
7. **Tidak ada** garis kiri (left-border) pada item aktif. Gunakan background saja.
8. Label teks: `font-size: 13px; font-weight: 500`. Menggunakan nama menu yang konsisten.

### Tombol Toggle Sidebar (`‹/›`)

Tombol untuk collapse/expand sidebar. Menggunakan `position: fixed` agar selalu terlihat di atas semua elemen.
...
| Ikon | Chevron `‹` (expanded) / `›` (collapsed), `14px`, `stroke: #334155` |
| Transisi posisi | `left 0.25s ease` (mengikuti lebar sidebar) |

**Kalkulasi posisi `left` tombol:**

```
left = (lebar sidebar - 14) + lebar sub-nav panel
```

| Kondisi | Sidebar | Sub-Nav | Posisi `left` |
|---|---|---|---|
| Expanded, tanpa sub-nav | 200px | 0px | 186px |
| Collapsed, tanpa sub-nav | 72px | 0px | 58px |
| Expanded, dengan sub-nav | 200px | 260px | 446px |
| Collapsed, dengan sub-nav | 72px | 260px | 318px |

```js
function toggleSidebar() {
  const sidebar = document.querySelector('.sidebar');
  const btn = document.querySelector('.sidebar-toggle-btn');
  const subnav = document.getElementById('subnavPanel');
  sidebar.classList.toggle('collapsed');
  const isCollapsed = sidebar.classList.contains('collapsed');
  const subnavWidth = subnav ? 260 : 0;
  const sidebarWidth = isCollapsed ? 72 : 200;
  btn.style.left = (sidebarWidth - 14 + subnavWidth) + 'px';
}
```
...
3. Tombol collapse (`<`) diposisikan di tepi kanan panel, tengah vertikal, berbentuk kotak `26×26px` dengan `border-radius: 6px`.
